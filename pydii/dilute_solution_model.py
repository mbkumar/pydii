...
def get_dC(M, n, site_species, specie_site_index_map):
        """
        ...
        Few cases: dC[i,i,i] = -1 due to being vacancy special case
                    ...
                    dC[i,j,k] = 0 if i!=j!=k
        """
        dC = np.zeros((M, M, n), dtype=np.int)
        for i in range(n):
            for j in range(n):
                for k in range(n):
...
def get_specie_conc(M, n, c0, dC, dE, specie_site_index_map, multiplicity,
                    mu, site_mu_map, beta):
    total_c = []
...
def get_conc_ratios(M, n, m, c0, dC, dE, specie_site_index_map, multiplicity,
                    mu, site_mu_map, beta):
...
def dilute_solution_model_without_sympy(structure, e0, vac_defs, antisite_defs,
    ...
    for i in range(m):
        ...
        #if specie_order[i] in  constrained_species :
        #    constrained_species_index_map = (low_ind, hgh_ind)
    els = [vac_def['site_specie'] for vac_def in vac_defs]

    dC = get_dC(M, n, site_species, specie_site_index_map)
    dE = get_dE(M, n, vac_defs, antisite_defs)
    ...
    # Compute composition range (values lie in between 0 to 100)
    stoic_comps = []
    for i in range(m):
        li = specie_site_index_map[i][0]
        hi = specie_site_index_map[i][1]
        comp = sum(multiplicity[li:hi])/sum(multiplicity)*100
    ...
    def get_ys(center_comps, indep_comp_min, indep_comp_max, delta):
        yvals = []
        for indep_comp in np.arange(indep_comp_min, indep_comp_max+delta,
                                    delta):
            comps = []
            for i in range(m):
    ...
    def find_mus(ys, guess):
        def conc_ratio_omega(mus):
            conc_rat = get_conc_ratios(M, n, m, c0, dC, dE,
                                       specie_site_index_map, multiplicity, mus,
                                       site_mu_map, beta)
            conc_rat.append(get_omega(M, n, dC, dE, e0, c0, multiplicity, mus,
                                      site_mu_map, beta))
            conc_rat = np.array(conc_rat)
    ...
    def compute_def_formation_energies(mu_vals):
        i = 0
        for vac_def in vac_defs:
            ...
            formation_energies['vacancies'][i]['formation_energy'] = \
                formation_energy
        ...
        for as_def in antisite_defs:
            ...
            formation_energies['antisites'][i]['formation_energy'] = \
                formation_energy
            specie_ind = site_mu_map[i]
            indices = specie_site_index_map[specie_ind]
            specie_ind_del = indices[1] - indices[0]
    ...
    if generate == 'energy':
        if trial_chem_pot:
            mus = [trial_chem_pot[element] for element in specie_order]
            ys = stoic_comps[1:]
            ys.append(0)
            ys = np.array(ys)/stoic_comps[0]
            mu_vals = find_mus(ys, mus)
        else:
            raise NotImplementedError("Chempot search is not implemented")

        formation_energies = compute_def_formation_energies(mu_vals)
    ...
    if trial_chem_pot:
        ...
        indep_comp_min = center_comps[independent_ind]-delta
        indep_comp_max = center_comps[independent_ind]-1
        delta = -delta
        yvals0 = get_ys(center_comps, indep_comp_min, indep_comp_max, delta)
        mus0 = []
        success_yvals0 = []
        trial_mu = init_mu #[-9.159,-2.947,-5.9318]
        ...
        # Try computing mus by using phase diagram approach at ideal stoichiometry
        # and use that to generate the mus
        raise NotImplementedError("Chempot search is not implemented")
    ...
    conc_data['x_label'] = els[independent_ind] + " mole fraction"
    ...
    en_data = {'x_label': els[independent_ind] + ' mole fraction', 'x': []}
    ...
    for as_def in antisite_defs:
        ...
        ind2 = specie_order.index(sub_specie)
        #specie_ind = site_mu_map[i]
        indices = specie_site_index_map[ind1]
        specie_ind_del = indices[1] - indices[0]
        cur_ind = i - indices[0] + 1
        if not specie_ind_del-1:
            label = '$' + sub_specie + '_{' + site_specie + '}$'
    ...
    mu_data = {'x_label': els[independent_ind] + ' mole fraction', 'x': []}
...
def compute_defect_density(structure, e0, vac_defs, antisite_defs, T=800,
    ...
    if plot_style == 'highcharts':
        ...
        def data_to_rows(inp_data):
            rows = []
            labels = []
            xlabel = '"' + inp_data['x_label'] + '"'
            labels.append(xlabel)
            labels += [y['name'].replace('$', '"') for y in inp_data['y']]
            #labels.sort()
...
@requires(sympy_found, "solute_site_preference_finder requires Sympy module. "\
        "Please install it.")
def solute_site_preference_finder(structure, e0, T, vac_defs, antisite_defs,
                                  solute_defs, solute_concen=0.01,
                                  trial_chem_pot=None):
...
def solute_site_preference_finder_without_sympy(structure, e0, T, vac_defs,
    ...
    # Initialization for concentrations
    # c(i,p) == presence of ith type atom on pth type site
    def get_site_conc(mu):
        c = np.zeros((n+1, n), dtype=np.float128)
        for i in range(n+1):
            for p in range(n):
                c[i, p] = c0[i, p]
                site_flip_contribs = []
    ...
    def get_host_site_conc(mu):
        host_c = np.zeros((n, n), dtype=np.float128)
        for i in range(n):
            for p in range(n):
                host_c[i,p] = c0[i,p]
                site_flip_contribs = []
    ...
    def get_conc_ratios(mu):
        total_c = get_specie_conc(mu)
        c_ratio = [total_c[i]/total_c[0] for i in range(1, m)]
        return c_ratio

    def get_host_specie_conc(mu):
        host_total_c = []
        host_c = get_host_site_conc(mu)
        for ind in specie_site_index_map[:-1]:
            val = 0
            for i in range(*ind):
                sum_i = sum([host_c[i,j] * multiplicity[j] for j in range(n)])
    ...
    def get_host_specie_conc(mu):
        host_total_c = get_host_specie_conc(mu)
        host_c_ratio = [host_total_c[i]/sum(host_total_c) for i in range(m-1)]
        return host_c_ratio

    # Expression for Omega, the Grand Potential
    # TODO: INVESTIGATE OMEGA DEFINITION
    def get_omega(mu):
        omega1 = e0 - sum([mu[site_mu_map[i]]*sum(c0[i,:])*multiplicity[i] \
                for i in range(n)])
        omega = omega1
        ...
        for p_r in range(n):
            for epi in range(n):
                sum_mu1 = sum([mu[site_mu_map[j]] * dC[j,epi,p_r] \
                               for j in range(n)])
                sum_mu = sum_mu1 - mu[site_mu_map[n]] * dC[n,epi,p_r]
                if p_r != epi and site_mu_map[p_r] == site_mu_map[epi]:
                    continue
    ...
    for site_range in specie_site_index_map:
        li = site_range[0]
        hi = site_range[1]
        comp = sum(mult[li:hi])/sum(mult)*100
        stoic_comps.append(comp)

    for i in range(m):
        print specie_order[i]
        if specie_order[i] not in constrained_species:
            independent_ind = i
            break
    #specie_concen = [
    #       sum(mult[ind[0]:ind[1]]) for ind in specie_site_index_map]
    max_host_specie_comp = 100 - solute_comp
    scaler = max_host_specie_comp / sum(stoic_comps)
    host_specie_stoic_comp = [comp * scaler for comp in stoic_comps]
    host_specie_stoic_comp[-1] = solute_comp
    # stoic_comp the range of 0-100
    # host_specie_comp in the range of 0-100
    ...
    def find_mus(ys, guess):
        def conc_ratio_omega(mus):
            conc_rat = get_conc_ratios(mus)
            conc_rat.append(get_omega(mus))
            conc_rat = np.array(conc_rat)
    ...
    if trial_chem_pot:
        if comps:
            # Check if the comps add up to 100 %
            sum_comp = sum(comps.values())
            sum_comp += solute_comp
            if sum_comp != 100.00:
                raise ValueError("Sum of compositions not equal to 100%")

            def get_ys_stoic_to_center(stoic_comps, center_comps):
                comp_diff = np.array(center_comps) - np.array(stoic_comps)
                ys = []
                for i in range(91):
                    y1s = list(np.array(stoic_comps) + i*comp_diff/90.0)
                    y1s.append(0)
            ...
            center_comps = [comps[specie_order[i]] for i in range(m)]
            center_comps.append(solute_comp)
            init_mu = [trial_chem_pot[element] for element in specie_order]
            yvals1 = get_ys_stoic_to_center(host_specie_stoic_comps,
                                            center_comps)
            trial_mu = init_mu #[-9.159,-2.947,-5.9318]
        else:
            center_comps = host_specie_stoic_comp
            init_mu = [trial_chem_pot[el] for el in specie_order]
...
def solute_site_preference_finder1_without_sympy(structure, e0, T, vac_defs,
    ...
    Args:
        ...
        solute_defs: List of solute defect parameters in the dictionary
            format. Similary to that of antisite defs, wtih solute specie
            specified in substitution_specie
        trial_chem_pot: Trial chemical potentials. Format is {el1: mu1, ...}
        comps: Composition of the compound. Format is {el1: comp, ...}
        constrained_species: In multicomponent systems other than binary,
            only two compositions are allowed to be varied. Specify the
            species whose composition is constrained to input values
    ...
    if not check_input(antisite_defs):
        raise ValueError('Antisite energy is not defined')
    if not check_input(solute_defs):
        raise ValueError('Solute energy is not defined')
...
def solute_defect_density_no_sympy(structure, e0, vac_defs, antisite_defs,
                                   solute_defs, T, trial_chem_pot, comps,
                                   constrained_species=[],
                                   plot_style="highcharts"):
    """
    Wrapper for the solute_site_preference_finder.
